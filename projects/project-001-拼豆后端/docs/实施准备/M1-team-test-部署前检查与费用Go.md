...
- [ ] 邮箱注册、验证、登录、登出、重设密码和邀请/白名单注册完成本地 API 与自动化测试。
- [ ] Google 登录本地 PoC 完成：模拟回调、会话、账号不隐式绑定、迁移与回滚验证通过。
- [ ] Work / WorkDocument / WorkAsset、50 个作品上限、上传边界、删除回收、A/B 隔离、Admin/Staff、限流和审计自动化测试通过。
- [ ] 文件和邮件的本地受控模拟已验证；没有真实用户文件、邮箱名单或凭据进入 Git、日志或测试样本。
- [ ] `pnpm lint`、类型检查、`pnpm test`、`pnpm build`、迁移状态、生产依赖审计和 Docker 部署检查全部通过。
- [ ] 前端联调所需的 API 契约与脱敏样本已确认，或已记录不阻塞 team-test 的明确剩余项。
