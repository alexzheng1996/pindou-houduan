...
import { withdrawCommunityPost } from '@/community/service'
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  try {
    ...
    await enforceAuthenticatedRateLimit(session, 'communityWrite', 'POST /api/v1/community/:id/withdraw')
    const result = await withdrawCommunityPost(session, id, sha256(requireIdempotencyKey(request)))
