...
import { createModerationNote } from '@/community/admin-service'
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  try {
    ...
    await enforceAuthenticatedRateLimit(session, 'communityModerationWrite', 'POST /api/v1/admin/community/users/:id/notes')
    return withBusinessCors(request, successResponse(await createModerationNote(session, (await context.params).id, await readBoundedJsonBody(request), sha256(requireIdempotencyKey(request))), requestId, 201), methods)
  } catch (error) {
