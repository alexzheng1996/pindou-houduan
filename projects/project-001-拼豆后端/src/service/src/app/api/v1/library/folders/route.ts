...
import { createFolder } from '@/library/service'
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    await enforceAuthenticatedRateLimit(session, 'libraryWrite', 'POST /api/v1/library/folders')
    const result = await createFolder(session, await readBoundedJsonBody(request), sha256(requireIdempotencyKey(request)))
