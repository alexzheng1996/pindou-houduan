...
import { getModerationCreator } from '@/community/admin-service'
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  try {
    ...
    await enforceAuthenticatedRateLimit(session, 'communityModerationRead', 'GET /api/v1/admin/community/users/:id')
    return withBusinessCors(request, successResponse(await getModerationCreator(session, (await context.params).id), requestId), methods)
  } catch (error) {
