...
import { copyCommunityPost } from '@/community/service'
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  try {
    ...
    await enforceAuthenticatedRateLimit(session, 'communityWrite', 'POST /api/v1/community/:id/copy')
    const body = await copyCommunityPost(session, id, sha256(requireIdempotencyKey(request)))
