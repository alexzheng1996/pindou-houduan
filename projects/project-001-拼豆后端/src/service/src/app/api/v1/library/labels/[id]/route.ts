...
import { deleteLabel, updateLabel } from '@/library/service'
...
const handle = async (request: Request, context: { params: Promise<{ id: string }> }, method: 'PATCH' | 'DELETE'): Promise<Response> => {
  ...
  try {
    ...
    await enforceAuthenticatedRateLimit(session, 'libraryWrite', `${method} /api/v1/library/labels/:id`)
    const result = method === 'PATCH'
      ? await updateLabel(session, id, await readBoundedJsonBody(request), sha256(requireIdempotencyKey(request)))
      : await deleteLabel(session, id, sha256(requireIdempotencyKey(request)))
