...
import { restoreModerationPost } from '@/community/admin-service'
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  try {
    ...
    await enforceAuthenticatedRateLimit(session, 'communityModerationWrite', 'POST /api/v1/admin/community/posts/:id/restore')
    return withBusinessCors(request, successResponse(await restoreModerationPost(session, (await context.params).id, await readBoundedJsonBody(request), sha256(requireIdempotencyKey(request))), requestId), methods)
  } catch (error) {
