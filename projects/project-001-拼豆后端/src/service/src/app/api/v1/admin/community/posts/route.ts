...
import { listModerationPosts } from '@/community/admin-service'
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    await enforceAuthenticatedRateLimit(session, 'communityModerationRead', 'GET /api/v1/admin/community/posts')
    return withBusinessCors(request, successResponse(await listModerationPosts(session, new URL(request.url).searchParams), requestId), methods)
  } catch (error) {
