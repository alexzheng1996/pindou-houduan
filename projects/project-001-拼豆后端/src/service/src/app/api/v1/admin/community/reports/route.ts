...
import { listModerationReports } from '@/community/admin-service'
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    await enforceAuthenticatedRateLimit(session, 'communityModerationRead', 'GET /api/v1/admin/community/reports')
    return withBusinessCors(request, successResponse(await listModerationReports(session, new URL(request.url).searchParams), requestId), methods)
  } catch (error) {
